...
给其他 agent / 脚本看的接口说明。**必填六项：菜图 + 菜名 + 窗口 + 饭堂 + 楼层 + 价格**，
其余都可以不传，系统会补默认值。
...
> **CI 约定**：上传者只写「自己那一个内容文件」，`contributions/index.json` 由
> `.github/workflows/data-index.yml` 自动重建；`pages.yml` 在部署前会**刷新**索引到产物里
> （不是检查是否最新——否则每次上传都会让部署失败，这一点踩过坑）。
> 所以 push 之后等约 1 分钟，线上就能看到（含图片）。
...
| | `""` `-` `未标注` `未知` `none` | `null` |
| | 其他（如 `9F`） | ❌ 报错「楼层无法识别」 |
| `price` | `12`（数字）、`"12"`、`"¥12"`、`"12-15"`、`"10元以下"` | 文本原样保留，数字则格式化为 `¥12` |
| | 不传 / 空 | ❌ 报错「缺少价格」 |
...
```json
{
  "records": [
    ...
    { "image": "assets/uploads/b.jpg", "name": "清炒时蔬", "window": "二楼自选", "canteen": "澜园", "floor": "1F", "price": "6" },
...
上传的菜会出现在这几处，**不用抽签也能看到**：

| 位置 | 看到什么 |
| --- | --- |
| 抽签页 · 「今日自选窗口」专区（舞台下方） | **一个窗口一张卡**：封面大图 + 窗口名 + 菜色清单（菜名+价格）+ 饭堂·楼层 + 「抽这一层」 |
| 抽签页 · 舞台下方小字 | `今日自选 N 道`，点它会滚动到上面的专区 |
| 抽签页 · 抽签结果 | 如果抽到该饭堂+楼层，会出现「今日自选」板块（带图） |
| 浏览页 · 「自选窗口」页签 | 列出所有自选窗口，点「看全部日期」进入窗口详情，菜色按日期倒序（今天/昨天/前天…） |
| 浏览页 · 饭堂详情 | 该饭堂「今天的自选窗口」+「窗口」横滑（窗口照片在卡片上） |

**所有菜图/窗口照片都可以点开看大图**（灯箱）：点图片打开全屏大图，
可按 ← → 或左右滑动翻看同组照片，Esc / 点背景 / 右上角 ✕ 关闭。

- 记录里的 `image` 直接作为 `<img src>` 使用，所以填 `assets/uploads/xxx.jpg`（相对站点根）或外链都能正常显示
- 上传的菜在「今日自选」「上传的菜」里是**16:10 大图**；在推送菜系列表里是 84px 缩略图
- 窗口照片显示在窗口卡片（横滑，152px 宽）
- 截图整理出来的 72 道常驻菜**没有图片**（原始截图里没有菜品图），所以只有上传的菜有图

> 每天 0 点后「今日自选」会清空（自选菜只当天有效），但菜不会丢 ——
> 到「浏览 → 上传的菜」里按日期还能翻到。
...
| **窗口的当日菜色** | 普通 `kind: "dish"`，但带 `date: "YYYY-MM-DD"` | **只在该日期有效**：当天进抽签池、进「今日自选窗口」；第二天自动退场 |
...
- 权重来源：帖子自带评价 —— 强烈推荐 5 / 好评·值得一试 3 / 两极 1.5 / 信息较少 1 /
  差评·已停业 0（且默认不进池）。
- 冷却：默认排除最近 5 次抽签的**主推菜**（只记当时推给你的那一道，不是整页推荐列表）；
  若整个池子都被冷却会自动放宽并提示。
- 舞台上的「池子 N 道 · M 个饭堂」是**稳定口径**（不含冷却），所以反复抽签数字不会变；
  冷却带来的变化单独显示为「已避开最近 K 道」。
  （早先版本直接显示冷却后的饭堂数，只有一道菜的饭堂会在被抽中后整店消失，
  看起来就像数字乱跳——已修，并加了回归测试。）
...
```bash
node tools/test_site_core.mjs          # 66 项：上传接口容错/裸格式文件/抽签可复现/公平性/自选菜日期语义/冷却口径
...
#   抽签页 14 + 管理台 10 + 抽签动效 6 + 自选菜快传 10 + 子路径部署 2 = 42 项
